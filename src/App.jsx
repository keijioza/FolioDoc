    ...
    <main className="main-content" aria-busy={busy || loading}>
      ...
      {active && user ? <DocumentEditor key={`${userId}:${active.id}`} document={active} user={user} users={users} onBack={() => navigate(view)} onSaved={remember} onDiscard={() => { guardRef.current = null; setActive(null); setReload(value => value + 1); }} guardRef={guardRef} /> : <>
        ...
        <div className="library-content">
          <div className="library-heading"><div><span className="eyebrow">A FRESH PAGE, A NEW POSSIBILITY</span><h1>{views[view][0]}<span className="heading-dot">.</span></h1><p>{views[view][1]}</p></div><button className="button import-button" onClick={() => uploadRef.current.click()} disabled={busy || !user}><Icon name="upload" size={17} />Import a file</button></div>
